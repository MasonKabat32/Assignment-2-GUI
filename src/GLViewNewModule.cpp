...
void GLViewNewModule::updateWorld()
{
   ...
   if (this->createSphere) { //check if flag to create a sphere was flipped
       addSphere(this->lastSphereHeight);
       this->createSphere = false; //reset flag
       this->lastSphereHeight += 8;
   }

   if (this->removeSphereFlag) { 
       removeSphere();
       this->removeSphereFlag = false;
   }
...
void GLViewNewModule::onKeyDown( const SDL_KeyboardEvent& key )
{
   ...
   if( key.keysym.sym ==  119) //W key
   {
       this->cam->moveInLookDirection();
   }
   else if (key.keysym.sym == 97) { //A key
       this->cam->moveLeft();
   }
   else if (key.keysym.sym == 115) { //S key
       this->cam->moveOppositeLookDirection();
   }
   else if (key.keysym.sym == 100) { //D key
       this->cam->moveRight();
   }
}
...
//this->moon = WO::New();  For Creation of the sphere
...
//Code for putting a texture on the sphere
//Tex tex = *ManagerTex::loadTexAsync( ManagerEnvironmentConfiguration::getSMM() + "/images/moonMap.jpg" );
    ...
    //There was one more line under this if the texture is not being loaded on properly


//Added functions for Assignment 2
void GLViewNewModule::addSphere(float height)
{
    WO* sphere = WO::New(); //This line might be a problem
    MGLIndexedGeometry* mglSphere = MGLIndexedGeometry::New(sphere);
    IndexedGeometrySphereTriStrip* geoSphere = IndexedGeometrySphereTriStrip::New(3.0f, 12, 12, true, true);
    mglSphere->setIndexedGeometry(geoSphere);
    sphere->setModel(mglSphere);
    sphere->setLabel("Added Sphere from gui");
    sphere->setPosition({ 15, 15, height });
    sphere->renderOrderType = RENDER_ORDER_TYPE::roTRANSPARENT;
    this->worldLst->push_back(sphere);
    Tex tex = *ManagerTex::loadTexAsync(ManagerEnvironmentConfiguration::getSMM() + "/images/moonMap.jpg");
    sphere->getModel()->getSkin().getMultiTextureSet().at(0) = tex; //exception happens here
    //Add a texture to the sphere
}
//
void GLViewNewModule::removeSphere() //Need a way to keep track of the spheres
{
    WO* dumpster = (this->worldLst->at(this->worldLst->size() - 1));
    this->worldLst->eraseViaWOIndex(this->worldLst->size() - 1);
    delete dumpster; //get rid of the workd object to prevent memory leaks
}
...
void Aftr::GLViewNewModule::loadMap()
{
...

   {
      ...
      light->setPosition( Vector( 0, 40, 100 ) );
   ...
   { //This code creates the plane
      ...
      this->gulfstream->setPosition( Vector( 0, 0, 25 ) );
   ...
   //this->gulfstream->setPosition(Vector(1, 1, 10));
...

   {
      ...
      auto show_moon_orbit_params   = [this]() { this->orbit_gui.draw(this->gulfstream, this->createSphere, this->removeSphereFlag); };
